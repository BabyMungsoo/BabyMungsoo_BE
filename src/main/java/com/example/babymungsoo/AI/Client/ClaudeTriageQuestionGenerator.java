...
public class ClaudeTriageQuestionGenerator implements TriageQuestionGenerator {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            선택지 만드는 규칙
            보호자는 응급 상황에서 문장을 칠 수 없습니다. 질문마다 눌러서 답할 선택지를 함께 만듭니다.
            - 질문마다 선택지를 2~4개 만듭니다. "잘 모르겠어요"는 넣지 않습니다.
              서버가 마지막에 항상 붙입니다.
            - 선택지는 서로 겹치지 않아야 하고, 응급도를 가르는 범위를 덮어야 합니다.
              (예: 횟수라면 "1번" / "2~3번" / "4번 이상" / "계속 토하고 있어요")
            - 보호자가 눈으로 확인할 수 있는 사실로 씁니다. 해석이나 용어를 쓰지 않습니다.
              (쓰지 않는 예: "호흡곤란"  →  쓰는 예: "숨 쉴 때 배가 크게 들썩여요")
            - 선택지에 응급도나 병명을 담지 않습니다.
              (쓰지 않는 예: "위험한 정도로 많이", "장염이 의심될 정도로")
            - 가벼운 것에서 심한 것 순서로 놓습니다.
            - 선택지 하나는 20자를 넘기지 않습니다.
            - "네 / 아니요"로 답할 질문이면 선택지도 "네" / "아니요" 두 개로 둡니다.
    ...
    // 출력은 질문 5개와 각 선택지가 전부다. 분석(2000)보다 작게 잡아 초과 과금을 막는다.
    // 선택지가 붙기 전 500이었고, 질문당 선택지 4개 × 20자를 더해도 1000이면 넉넉하다.
